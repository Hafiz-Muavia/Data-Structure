...
if(s<5){
    ...
    cout << "<<<<<<-------------------------------------------PRINTING FINAL DATA ------------------------------------------------------>>>>>>" << endl;
     ofstream file("D:/SEM 4/DSA/PROGRAMS/assignments/3/text/output.txt");  // Open the file for writing
     file << "<<<<<<-------------------------------------------RESULT SHEET OF STUDENTS ------------------------------------------------------>>>>>>" << endl;
    print(file);  // Pass the file object to the print() function
    file.close();
}
